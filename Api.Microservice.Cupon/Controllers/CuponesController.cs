...
namespace Api.Microservice.Cupon.Controllers
{
    ...
    public class CuponesController : ControllerBase
    {
        ...
        public async Task<ResponseDto> Get()
        {
            try
            {
                var cupones = await _appDbContext.Cupons.ToListAsync();
                var currentDate = DateTime.Now;

                foreach (var cupon in cupones)
                {
                    // Si currentDate es mayor que FechaFinal, cupón no está activo
                    cupon.Status = (currentDate > cupon.FechaFinal) ? false : true;
                }
